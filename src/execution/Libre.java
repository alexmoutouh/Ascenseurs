...
public class Libre {
	...
	public static void trier(Ascenseur ascenseur) {
		// appel de l'algo de tri des requetes de l'ascenseur.
		triReq.trier(ascenseur);
	}
	...
	/**
	 * Ajoute une requete externe. Pour cela on, fait un appel a choisirAscenseur()
	 * qui va derterminer l'ascenseur pour cette requete.
	 */
	public void creerRequeteExterne(int etageRequeteExterne, String direction) throws IOException {
		...
		/********************************************************************************
		 * Condition pour ne pas avoir l'etage de la requete externe superieur au nombre
		 * d'etage ou inferieur a 0.
		 ********************************************************************************/
		if (etageRequeteExterne <= Controleur.getInstance().getAscenseurs().get(0).getNombreEtage() - 1
			...
			if (direction.equals("haut") || direction.equals("bas")) {
				...
				/*******************************************************
				 * On effectue un nouveau tri pour tous les ascenseurs.
				 *******************************************************/
	...
	public void bloquerOuDebloquer(EtatAscenseur etatSauve, int numeroAscenseur)
		...
		/****************************************************************************
		 * Si l'utilisateur appuie une premiere fois pour un ascenseur choisi, on le
		 * bloque puis s'il appuie une deuxieme fois pour le meme ascenseur on le
		 * debloque grace a la sauvegarde de l'etat.
		 ****************************************************************************/
		System.out.println("Quel ascenseur voulez vous bloquer/debloquer ? ");
		numeroAscenseur = Integer.parseInt((bufferReader.readLine()));
		if (Controleur.getInstance().getAscenseurs().get(numeroAscenseur - 1).getEtat() != Constante.KBloquer()) {
			etatSauve.addMemento(Controleur.getInstance().getAscenseurs().get(numeroAscenseur - 1).sauverMemento());
			Controleur.getInstance().getAscenseurs().get(numeroAscenseur - 1).bloquer();
		} else {
			Controleur.getInstance().getAscenseurs().get(numeroAscenseur - 1).debloquer(etatSauve.getMemento());
		}
	}

	/**
	 * Ajoute une requete interne a un ascenseur que l'utilisateur va determiner. On
	 * trie ensuite grace a l'algo de tri.
	 */
	public void creerRequeteInterne(int etageRequeteInterne, int numeroAscenseur)
		...
		/*****************************************************************************
		 * Condition pour pas avoir l'etage de la requete externe superieur au nombre
		 * d'etage ou inferieur a 0.
		 *****************************************************************************/
		if (etageRequeteInterne <= Controleur.getInstance().getAscenseurs().get(0).getNombreEtage() - 1
			...
			if (numeroAscenseur <= nombreAscenseur) {
				...
				triReq.trier(Controleur.getInstance().getAscenseurs().get(numeroAscenseur - 1));
			} else {
	...
	public void execIter() throws IOException {
		...
		/*****************************************************************************
		 * Dans cette fonction, la boucle principale va executer les iterations du
		 * programme, et donc, a chacune d'entre elles, on demande a l'utilisateur de
		 * faire un choix, puis on appelle les fonctions adequates en consequence.
		 *****************************************************************************/

		/*******************************************************************************
		 * Declaration et initialisation des variables avant la boucle afin de pas les
		 * redeclarer a chaque iteration.
		 *******************************************************************************/

		/*******************************************************************************
		 * etatSauve est une variable qui permet de sauver l'etat avant de bloquer
		 * l'ascenseur puis de la restituer lors de l'appel de debloquer
		 *******************************************************************************/
		...
		while (!choix.equals("Q")) {
			System.out.println("Appuyez sur E pour ajouter une requete externe : ");
			System.out.println("Appuyez sur I pour ajouter une requete interne : ");
	...
	public void initialisation() throws IOException {
		/*****************************************************************************************
		 * Debut du programme. 
		 * On rentre dans une boucle qui se termine seulement si le programme n'a pas genere 
		 * d'exception. 
		 * Dans cette partie on initialise les ascenseurs (avec des options ou pas), on demande 
		 * egalement a l'utilisateur de rentrer le nombre d'etages et le nombre d'ascenseurs 
		 * puis on appelle la fonction dans laquelle on effectue les iterations. 
		 * L'utilisateur a egalement la possibilite de choisir entre 2 affichages (IHM ou console) 
		 *****************************************************************************************/
		...
		case "C":
			...
			for (int i = 0; i < nombreAscenseur; ++i) {
				System.out.println("Entrez V pour un ascenseur avec une vitesse accrue");
				System.out.println("Entrez M pour un ascenseur avec une musique de fond");
				System.out.println("Entrez N pour un ascenseur normal sans option");
				choixOption = bufferReader.readLine();

				switch (choixOption.toUpperCase()) {
				case "M":
					mesAscenseurs.add(new OptionMusique(new Ascenseur(nombreEtage)));
					break;
				case "V":
					mesAscenseurs.add(new OptionVitesse(new Ascenseur(nombreEtage)));
					break;
				case "N":
					mesAscenseurs.add(new Ascenseur(nombreEtage));
					break;
				default:
					System.out.println("Entrer errone on creer un ascenseur sans option");
					mesAscenseurs.add(new Ascenseur(nombreEtage));
				}
