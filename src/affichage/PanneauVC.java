	...
	public void paintComponent(Graphics g) {
		...
		for (int i = 0; i < etage; ++i)
			for (int j = 0; j < nbAscenseur; ++j)
				try {
					grille[i][j] = ImageIO.read(new File("img/Nasc.jpg"));
				} catch (IOException e) {
					e.printStackTrace();
				}
		...
		for (Ascenseur as : Controleur.getInstance().getAscenseurs()) {
			if (as.getEtat() == Constante.KFerme())
				try {
					grille[etage - 1 - as.getEtageCourant()][nbas] = ImageIO.read(new File("img/ascClose.jpg"));
				} catch (IOException e) {
					e.printStackTrace();
				}
				...
				try {
					grille[etage - 1 - as.getEtageCourant()][nbas] = ImageIO.read(new File("img/ascOpen.jpg"));
				} catch (IOException e) {
					e.printStackTrace();
				}
