		...
		if (direction != "ouvrir") { // si l'asc doit se deplacer
			for (Requete req : ascenseur.getAscenseur().getRequetes()) { // pour
				...
				if (req.getEtageDeLaRequete() > ascenseur.getAscenseur()
					...
					ReqDescend.add(req);
				} else { // rappel : on doit se deplacer ; cas etage destination
							// =
			...
			Collections.sort(ReqMonte, new Comparator<Requete>() {// on trie par ordre
														// croissant
				@Override
			...
			Collections.sort(ReqDescend, new Comparator<Requete>() { // on trie par ordre
														// decroissant
						@Override
						public int compare(Requete req1, Requete req2) {
