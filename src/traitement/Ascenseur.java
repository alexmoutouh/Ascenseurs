...
public class Ascenseur implements IAscenseur {
	...
	private int index; // emplacement de l'ascenseur dans le tableau du contoleur
	...
	public void supprDoublons() {
		...
		while (iterRequete.hasNext()) {
			...
			while (iterRequeteBis.hasNext()) {
				...
				if ((requeteAVerifie.getEtageDeLaRequete() == requeteAComparer.getEtageDeLaRequete()
						&& (doublon == true))) {

					iterRequete.remove();
					break;
				} else if ((requeteAVerifie.getEtageDeLaRequete() == requeteAComparer.getEtageDeLaRequete())) {
					doublon = true;
				}
			}
		}
	...
	public void action() {
		...
		if (this.etat != Constante.KBloquer()) {
			/****************************************************************************
			 * Premiere condition : on ferme l'ascenseur si l'etat actuel est ouvert, car
			 * apres s'etre ouvert il doit forcement fermer.
			 ****************************************************************************/
			if (this.etat == Constante.KOuvert() || req == null || this.etat == null) {
				fermer();
			}
			 ...
			 *************************************************/
			else if ((this.etat == Constante.KFerme() || this.etat == Constante.KDescend())
			 ...
			 **********************************************/
			else if ((this.etat == Constante.KFerme() || this.etat == Constante.KMonte())
					&& (req != null && req.getEtageDeLaRequete() > this.etageCourant)) {
			 ...
			 ******************************************************************/
			else if (req != null && req.getEtageDeLaRequete() == this.etageCourant) {
				ouvrir(req);
			}

			else {
				System.out.println(req.getEtageDeLaRequete());
				System.out.println("Requete inconnue et ignoree");
			}
			...
			System.out.println("L'ascenseur est bloque et ne peut pas effectuer d'action.");
		}
	}
	...
	/**
	 * restorerMemento() : Recupere l'etat de l'ascenseur avant que celui-ci soit
	 * bloque.
	 */
	 ...
	 * remonter grace a la recursivite jusqu a l'objet decore dans les options. Sa
	 * principale utilite apparait lors de l'utilisation d'un TriRequete
	 */
